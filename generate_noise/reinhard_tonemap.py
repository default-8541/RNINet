...
def parse_args():
    ...
    parser.add_argument('-i', '--in_dir', default='testing', type=str)
...
if __name__ == "__main__":
    ...
    for i, file in enumerate(tqdm(files[:], ascii=True)):
        ...
        command = 'luminance-hdr-cli -l %s --tmo reinhard02 --tmoR02Key %f -G %f -S %f -o %s > /dev/null' % (
        os.path.join(src_path, file), key, tm_gamma, sat_fac, tmp_path)
#         command = 'luminance-hdr-cli -l %s --tmo reinhard02 --tmoR02Key %f -G %f -S %f -o %s > nul' % (
#         os.path.join(src_path, file), key, tm_gamma, sat_fac, tmp_path)
        os.system(command)
